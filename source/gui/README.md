...
1. **`simplify_expression()`**
    ...
    - **Процес:**
        - Взема избрания израз чрез `get_active_expression()`.
...
5. **`decompose_expression()`**
    ...
    - **Процес:**
        - Взема израза и променливата за декомпозиция чрез `get_active_expression()` и `variable_entry.get()`.
        - Валидира израза.
        - Създава обект `BooleanFunction` и го добавя към `function_set`.
...
8. **`visualize_ast()`**
    ...
    - **Процес:**
        - Взема и валидира израза.
        - Създава обект `BooleanFunction` и го добавя към `function_set`.
        - Взема AST чрез `boolean_function.ast`.
...
9. **`check_equivalence()`**
    ...
    - **Процес:**
        - Взема двата израза от `first_expression_entry` и `second_expression_entry`.
...
12. **`resource_path(relative_path)`**
    - **Цел:** Осигурява правилното намиране и зареждане на ресурси.
    - **Процес:**
        - **`getattr(sys, "_MEIPASS", os.path.dirname(__file__))`:**
            - Функцията използва `getattr` за да провери дали атрибутът `_MEIPASS` съществува в модула `sys`.
            - **`sys._MEIPASS`:** Този атрибут указва директорията, в която PyInstaller извлича временно ресурсите по време на изпълнение на  изпълнимия файл.
            - **`os.path.dirname(__file__)`:** Ако атрибутът `_MEIPASS` не съществува, функцията използва директорията, в която се намира текущият скрипт `__file__`.
        - **`base_path`:** Съдържа пътя до директорията, където се намират ресурсите, независимо от начина на изпълнение.

---

### **Помощни Функции**

1. **`get_active_expression()`**
    - **Цел:** Връща избрания логически израз (първи или втори).
    - **Процес:**
        - Проверява стойността на `active_expression`.
        - Връща съдържанието на съответното поле за въвеждане `first_expression_entry` или `second_expression_entry`.
...
1. **`open_sets_window()`**
    ...
    2. **Вътрешни Функции в `open_sets_window()`**
        ...
        - **`bitmask_to_set(bitmask, all_elements)`**
...
1. **`run()`**
    ...
    - **Процес:**
        ...
        - Създава елементи за въвеждане на два логически израза `first_expression_entry`, `second_expression_entry` и радио бутони за избор на активен израз.
