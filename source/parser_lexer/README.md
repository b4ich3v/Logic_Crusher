...
Итерира през входния текст и генерира списък от `Token` обекти въз основа на дефинираните спецификации за токени.

- **Компоненти:**
   - **Инициализация:**
      - `tokens = []`: Инициализира празен списък за съхранение на токените.
   - **Итерация през Съвпаденията:**
      - `re.finditer(self.token_regex, self.text)`: Сканира входния текст и връща итератор от обекти за съвпадение `token_match` за всяко съвпадение на дефинираните токен           модели.
   -  **Обработка на Всяко Съвпадение:**
      - `kind = token_match.lastgroup`: Взема името на последната съвпаднала именувана група, което указва типа на токена.
      - `value = token_match.group(kind)`: Извлича действителния подстринг от входа, който е съвпаднал с модела на токена.
...
Представлява синтактичен анализатор, който приема списък от токени, генерирани от лексера, и изгражда абстрактно синтактично дърво (AST) въз основа на определена граматика за логически изрази.
...
1. **Инициализация `__init__`:**
    ...
    - **Компоненти:** 
      ...
      - `position`: Текущата позиция в списъка с токени. Започва от `0`.
...
2. **Метод `eat`:**
    ...
    - **Процес:**
      ...
      - Ако съвпада:
         - Увеличава позицията `position` с 1.
...
### **Стъпки на Парсване:**               
1. `expr()` извиква `equiv_expr()`.
2. `equiv_expr()` извиква `imp_expr()`.
3. `imp_expr()` извиква `or_expr()`.
4. `or_expr()` извиква `xor_expr()`.
5. `xor_expr()` извиква `and_expr()`.
6. `and_expr()` извиква `nand_expr()`.
7. `nand_expr()` извиква `factor()`.
8. `factor()` открива `NOT`:
   - Потвърждава `NOT` и премества към следващия токен `(`.
   - Рекурсивно извиква `factor()` за обработка на `(A AND B)`:
     - Потвърждава `LPAREN` и извиква `expr()`.
     - Процесът се повтаря за `A AND B`, изграждайки съответните възли.
   - Създава `NotNode` с вътрешния израз.
9. Продължава с `OR C`, създавайки `OrNode` с предишния `NotNode` и `VariableNode('C')`.
10. Добавя `EOF` и връща кореновия възел на AST.
