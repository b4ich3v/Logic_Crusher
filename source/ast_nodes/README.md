...
- **Методи:**
    ...
    - `evaluate(variables)`: Изчислява стойността на израза при дадено присвояване на променливите (`variables`).
    ...
    - `substitute(variables)`: Заменя променливите в израза с константи, ако са зададени в `variables`.
    - `to_graphviz(graph, counter)`: Добавя възела в Graphviz графа за визуализация на израза.
...
1. **`VariableNode`**
    ...
    - При оценка връща стойността на променливата от дадената среда (`variables`).
    - При преобразуване в Жегалкинова форма, представя променливата като мононим с една променлива.
    - При заместване, ако променливата има зададена стойност в `variables`, я заменя с константа (`ConstNode`), иначе остава непроменена.
...
Всеки клас има метод `evaluate(variables)`, който изчислява стойността на израза при дадено присвояване на променливите (`variables`):

- **Променливи**: Връщат стойността им от `variables`.
...
Методът `substitute(variables)` позволява заместване на променливите с константи според дадена среда (`variables`). Например, ако имаме израз `A AND B` и `env = {'A': True}`, то след заместване изразът става `True AND B`, което може да се опрости до `B`.

### **Метод за визуализация (`to_graphviz`)**

Всеки клас има метод `to_graphviz(graph, counter)`, който добавя текущия възел към Graphviz графа за визуализация на израза. Това е полезно за генериране на графични представяния на Булевите изрази, което може да помогне при анализа и дебъгването.
